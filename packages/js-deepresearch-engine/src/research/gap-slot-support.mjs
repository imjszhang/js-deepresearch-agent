...
export function selectSlotPassages(gap, findings = [], { topK = 3 } = {}) {
  ...
  return collectSuccessfulPassages(findings, { gapId: gap.id, allowFallback: true })
    .map((passage) => ({
      ...passage,
      retrievalScore: tokenOverlapScore(focus, passage.text),
    }))
...
export async function judgeOpenSlotSupport({
  ...
  if (!targets.length) {
    return { judgments: [], unknown: false, retried: false };
  }

  const attachFailClosed = (reason) => targets.map(({ gap }) => ({
  ...
  if (!llm?.complete) {
    return { judgments: attachFailClosed('no_llm'), unknown: true, retried: false };
  }
  ...
  try {
    const result = await completeStructuredJson({
      ...
      maxTokens: 800,
      retryMaxTokens: 600,
      accept: hasUsableSupportPayload,
      messages: gapSlotSupportPrompt({ query, slots: targets }),
      retryMessages: gapSlotSupportPrompt({ query, slots: targets, compact: true }),
    });
    if (!result.ok) {
      return { judgments: attachFailClosed(result.reason || 'invalid_or_empty_json'), unknown: true, retried: result.retried };
    }
    const rawJudgments = judgmentsFromParsed(result.parsed);
    const judgments = targets.map(({ gap, passages }) => {
      const matched = matchJudgment(gap, rawJudgments)
        || (targets.length === 1 && rawJudgments.length === 1 ? rawJudgments[0] : null);
      if (!matched) {
        return { ...failClosedSupport('unmatched_judgment'), gapId: gap.id, answerSlot: gap.answerSlot, question: gap.question };
      }
      return {
        ...normalizeJudgment({ ...matched, gapId: gap.id, answerSlot: gap.answerSlot, question: gap.question }, passages),
        gapId: gap.id,
    ...
    return { judgments: attachFailClosed(error?.message || 'judge_error'), unknown: true, retried: false };
  }
}
