...
Executed search queries come only from the original user query or the shared LLM Search Query Planner (`purpose: search_query_planning`). Planner items may include optional `searchOptions` (`engines`, `categories`, `language`, `pageno`); those values are type-checked and filtered by provider `capabilities.supportedSearchOptions`. SearXNG continues to declare the full option set. Fixed-engine providers such as JS Eyes Google ops drop unsupported options and record `requestedSearchOptions` vs `effectiveSearchOptions` on SearchMeta instead of pretending to honor them. Typed provider errors (`rate_limited`, `provider_error`) stay out of semantic `repairFailures`. `quality.readiness` and `quality.completionStatus` are the evidence-contract signals; legacy `quality.gate` remains a report-quality label. Required-slot evidence stays dedicated unless a successful body is explicitly promoted after that slot's own relevance check. Optional brief `asOf` and `entityAliases` come from the user or structured planner output only. Strategies may schedule, validate, deduplicate, and stop; they do not splice hosts, detect language, pick engines, or strip `site:` to invent a new query. Invalid or exhausted planner output skips the wave or blocks the gap (`quality.stopDetail=query_planner_exhausted`) instead of falling back to rule templates. Every executed search carries `queryOrigin` of `user_query` or `llm_planner`, plus provider observations on the `Source[]` metadata symbol.
...
Focused and exploratory runs also own `TransportMemory`. It remembers failed `(URL, backend, retrievalPath)` routes while leaving successful reads eligible for a later fresh fetch, and opens the HTTP-direct host circuit after `research.read.transport.hostCircuitThreshold` consecutive `403`, `429`, or deterministic challenge attempts (default `3`; `0` disables the circuit). Internal `429` attempts count individually through `fetchAttempts`. Switching backend or retrieval path remains eligible. The fetcher retries only `408`/`429`, timeout, `5xx`, and network failures, consumes bounded `Retry-After`, and never retries `AbortError`. Memory snapshots support serialization/round-trip in strategy checkpoints. `ResearchRunner.resumeFromSession()` can finish a report from `pre-report`, continue an unfinished exploratory loop from `exploratory-step-complete`, or continue exploring when callers pass `continueExplore` plus `extraSteps >= 1`.

`research.read.transport.responseHeadersTimeoutMs` (default `10000`) bounds the fetch API's response-header promise; this is the observable boundary and is not claimed to be a separately measured first body byte. `htmlTotalTimeoutMs` (default `15000`) and `documentTotalTimeoutMs` (default `60000`) independently bound the full request/body/conversion operation. URLs or responses identified as documents, plus responses above `largeFileThresholdBytes` (default 5 MiB), use the document total timeout. Results record `timeoutStage` and `timeoutPolicy` for audit. `maxAttempts` and `largeFileThresholdBytes` are programmatic settings only, without dedicated CLI flags or environment variables.
...
Report synthesis validates non-empty Markdown output with `research.reportValidation.minChars` (default `200`) and `maxAttempts` (default `2`). Before validation it deterministically removes exact numeric `[gap-N]` references, leading complete `<think>...</think>` reasoning prefixes, leading orphan `</think>` tags, and empty list items, then re-renders the canonical narrative. Ordinary bracket text and embedded tagged examples are preserved; embedded thinking tags fail render validation rather than silently turning hidden text into a fact. Every verified required slot must retain its own anchored, cited, slot-bound plan claim, with deterministic supplementation from frozen evidence when the LLM omits it. Remaining failures are classified as `provider`, `parse`, `semantic-contract`, or `render`; malformed structured output has an independent retry counter and does not consume semantic retries. `ReportGenerationError` and `failure.json` expose safe structured `failedChecks` values and the final `phase`; free report lines, prompts, reasoning, and provider text are replaced by lengths/counts/enums/hashes and never serialized into the error message. Structured LLM telemetry records only safe operational metadata. Focused runs also emit gaps and rule-based limitations, focus follow-up searches on missing primary evidence, and only build direct passages from source bodies whose fetch completed successfully.
