...
Report synthesis validates non-empty Markdown output with `research.reportValidation.minChars` (default `200`) and `maxAttempts` (default `2`). Before validation it deterministically removes internal `[gap-*]` references, `<think>` tag residue, and empty list items, then re-renders the canonical narrative. Remaining failures are classified as `provider`, `parse`, `semantic-contract`, or `render`; malformed structured output has an independent retry counter and does not consume semantic retries. `ReportGenerationError` and `failure.json` expose `failedChecks` entries with expected/actual values and the final `phase`, rather than reporting the character minimum when it passed. Structured LLM telemetry records only safe operational metadata. Focused runs also emit gaps and rule-based limitations, focus follow-up searches on missing primary evidence, and only build direct passages from source bodies whose fetch completed successfully.
