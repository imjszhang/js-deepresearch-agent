...
- `exploratory` (探索性调研): budget-driven agent loop. The model chooses structured search/read/reflect/answer actions while the runtime guards legality, cancellation, token floor/ceiling, and evidence sufficiency. Token ceilings apply to exploration only; the final report is assembled separately and is not clipped by the 1200-token reserve. Search/read count caps and maxSteps default to unlimited; a step safety valve applies only when both token ceilings are off.

`focused` defaults to a quality preset: `fetchMode: summary`, query memory, source clustering, passage/claim evidence, iteration/evidence early-stop, and soft budgets (`maxSearchRequests: 18`, `maxSourceReads: 16`). Those count budgets apply to `focused` / `quick` only. Override per run with CLI flags such as `--focused-fetch-mode disabled` or `config set` / Web UI settings. `preReportGate` and LLM relevance filtering remain off by default.
