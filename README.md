...
- `exploratory` (探索性调研): budget-driven agent loop. The model chooses structured search/read/reflect/answer actions while the runtime guards legality, cancellation, token floor/ceiling, and evidence sufficiency. Search/read count caps and maxSteps default to unlimited; a step safety valve applies only when both token ceilings are off.
