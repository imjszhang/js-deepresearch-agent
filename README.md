...
The existing `benchmark`, `benchmark:strategies` and `benchmark:extract` entry points now dispatch quality subcommands to the same implementation. For example, `npm run benchmark -- score ...` uses exactly the independent evaluator and execution gates described above.

Existing session/ID inspection is offline by default and reports versioned artifact integrity and observable counts. It never loads model settings, reuses stored semantic verdicts, or turns keyword overlap into a support score. `--no-llm` remains a compatibility alias.
...
Inspection output uses schemaVersion 2 and separates `artifactVerification` from `modelAssessment`. The latter remains unobserved with null thresholds until an explicit independent score is produced. V2 artifacts use the pinned manifest, bodies and citation registry. Archived runs use their committed revision and snapshot evidence. Corrupt versioned artifacts fail; old four-file bundles and standalone JSON without a verifiable manifest remain incomplete. Successfully returning a diagnostic record does not imply that its artifact status passed.

Strategy comparison retains recorded cost and labels the former query-battery audit as `legacy_heuristic_diagnostics`, with `authoritative=false`. Its regex-derived slots and ready/not_ready status are not independent research quality or program acceptance. Missing costs are n/a; validation tokens do not satisfy the exploration floor. Explicit multi-strategy `--run` requires a current `--program-verification` record before loading research settings; use the shared `plan / run / score` commands for a frozen quality campaign.
...
The quality benchmark separates program verification, artifact integrity, and model assessments. Run `npm run verify:programmatic -- --output-dir work_dir/programmatic-verification/my-check` to execute the offline regression/scenario suite, lint, build and diff checks without model credentials. A successful record verifies program behavior under the tested contracts, including wrong model responses; it does not prove semantic accuracy. `score --mode model-observation --program-verification <file>` and `rebuild --program-verification <file>` use this record without requiring real-model calibration accuracy. Legacy calibrated commands and improvement gates retain their original strict meaning. Ordinary research CLI/API use is unchanged. Use `npm run benchmark:quality -- --help` for commands.
...
The subsequent review fixes deduplicate task/claim bindings, preserve startup response recovery across repeated interruptions, and add independent line-by-line extraction coverage review. The v4 calibration failed: its new holdout matched 14/20 cases, with three omitted factual headings correctly held for review and unresolved evidence-silence errors. See the [review fix record](journal/2026-09-11/review-fixes.md) and [v4 calibration results](journal/2026-09-11/calibration-v4-results.md).
