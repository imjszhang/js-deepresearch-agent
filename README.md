...
- `focused` (专题调研, default): builds a versioned `ResearchBrief`, runs bounded discovery, merges and reads evidence, then searches only material open claim/answer-slot gaps. Consequential or critical slots receive one bounded challenge and a source-level spot-check.
...
`ResearchRunner.run()` accepts the existing string `query` or a structured brief object. Planner output is deterministically sanitized; absent fields stay empty, and answer-slot required hosts are accepted only when the hostname is literal in the query. `ResearchBrief.schemaVersion` is `1`; claim/slot gaps use schema version `2` and the states `open`, `searched`, `body_read`, `verified`, `conflicting`, and `limited`. Snippets cannot verify focused/exploratory gaps.
