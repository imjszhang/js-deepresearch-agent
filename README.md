...
The report is derived from the final research state (`ReportContract` / `ReportPlan`). Markdown headings and cross-section dedup cannot overturn a verified judgment slot. Rendering problems are recovered deterministically; `REPORT_OUTPUT_INVALID` is reserved for empty/unparseable provider output or a completed contract that still has no supportable answer. The engine still requires a Markdown heading and at least `research.reportValidation.minChars` characters (default `200`). Failed generation writes no report artifacts and the CLI/Web history state is `failed`. LLM progress records purpose, duration, output length, finish reason, and whether reasoning metadata existed, but never stores prompts or reasoning text.
...
- `exploratory` (探索性调研): budget-driven Search-Read-Reason loop. The deterministic readiness gate, cancellation, and token floor/ceiling remain authoritative. Low novelty and repeated results redirect the next search angle, but plateau cannot cross the token floor or turn a failed gate into `evidence_sufficient`. Recovery escalates through unread candidates, strict slot queries, relaxed slot queries, and an LLM rewrite. A gap is blocked after three **semantic** failed recovery rounds by default; transient provider errors and duplicate queries do not increment `repairFailures`. All blocked gaps or six consecutive semantic no-yield steps stop with `safety_cap` and `quality.stopDetail="repair_exhausted"`. `quality.completionStatus` is `incomplete` when a safety cap leaves required slots open; `quality.gate` remains the legacy report-quality label and is not the readiness signal. Required-slot bodies stay isolated unless a successful body is explicitly promoted to another slot after that slot's own relevance check. Brief `asOf` and `entityAliases` are optional structured fields from the user or planner.
